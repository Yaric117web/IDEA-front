        ...
        case 'button':
            ...
            if (config['mode'] == 'button') {
                ...
                $window.on('load scroll', function () {
                    if (!pdoPage.Reached && $window.scrollTop() + 400 > wrapper.height() - $window.height()) {
...
pdoPage.loadPage = function (href, config, mode) {
    ...
    $.post(config['connectorUrl'], params, function (response) {
        if (response && response['total']) {
            ...
            $(document).trigger('pdopage_empty', [config, response]);
        }
    ...
    set: function (vars) {
        ...
        if (!this.oldbrowser()) {
            ...
            window.history.pushState({
                pdoPage: document.location.pathname + hash
